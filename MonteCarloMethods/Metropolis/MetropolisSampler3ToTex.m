FID = fopen('MetropolisSimulationImage.tex', 'w');
fprintf(FID, '\\begin{figure}\\label{fig: SimulationMetropolis0}\n');
...
if(mod(i,8)==1 && i>1)
    ...
    fprintf(FID, '\\begin{figure}\\label{fig: SimulationMetropolis%d}\n',j); 
end  
...
%fprintf(FID, '\\includegraphics[width=0.5\\textwidth]{ImaginiLatex/%s}\\\\ \n',fileimg);
fprintf(FID, '\\textbf{Simulation %d} $\\theta_0=%8.2f$  $\\sigma=%8.2f$  & ',i, theta(i,1), sigma(i));
fprintf(FID, '\\textbf{Simulation %d} $\\theta_0=%8.2f$  $\\sigma=%8.2f$\n',i+1,theta(i+1,1), sigma(i+1));
