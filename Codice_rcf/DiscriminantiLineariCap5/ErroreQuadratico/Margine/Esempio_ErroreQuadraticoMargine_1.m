% Esempio_ErroreQuadraticoMargine_1.m
%
...
%
% NB: Ricerco la retta di Separazione migliore attraverso l' ALGORITMO 
%     DEL GRADIENTE DISCENDENTE IMPLEMENTATO DA ME...
%
%
...
[J DJ]=ErroreQuadraticoMargine(a.',V,b);
J=reshape(J,M,N);
surf(a1,a2,J)
xlabel('a1');
ylabel('a2');
...
%Calcolo il minimo ErroreQuadraticoMargine
[a Val]= fminsearch(@(x) ErroreQuadraticoMargine(x,V,b),[1;1],{V,b});
...
ao=[10 -21];
a=ao;
plot3(a(1),a(2),ErroreQuadraticoMargine(a.',V,b),'om','MarkerSize',5,'MarkerFaceColor','m')
k=1;
DJ=100;
while norm(lr*DJ,2)> tol && k<100
      
      %Calcolo il gradiente
      [J DJ]=ErroreQuadraticoMargine(a(k,:).',V,b);      
      % norm(lr*DJ,2)
      ...
      [Jnew DJnew]=ErroreQuadraticoMargine(a(k,:).',V,b);
