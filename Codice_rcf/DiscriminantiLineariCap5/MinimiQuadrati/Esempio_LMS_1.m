...
iter=0;
maxiter=1000;
%Matrice degli Incrementi
exit=ones(1,n);
while (all(exit)>tol  && iter <maxiter) 
    k=mod(k+1,n)+1;
    lr=1/k;
    
    Inc=lr*(b(k)-(a.'*V(:,k)))*V(:,k);
    
    a=a+lr*Inc;
    
    %Calcolo la condizione di uscita per ogni Training Pattern
    for K=1:n
       exit(K)=norm(lr*(b(K)-(a.'*V(:,K)))*V(:,K));
    end
