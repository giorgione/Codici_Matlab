...
for t=1:T-1
   ...
   for j=1:N
        ...
        for i=1:N
            S=S+Alfa(t,i)*A(i,j);
            display([ 'Alfa(' num2str(t) ',W' num2str(i-1) ')*A( W' num2str(i-1) ', W' num2str(j-1) ...
                ') ->' num2str(Alfa(t,i)) ' * ' num2str(A(i,j)) '+'] )
        end
        display([ '= ' num2str(S) '-> Probabilità che si Verifichi lo STATO NASCOSTO W' num2str(j-1) ]);
        %gli indici in V vanno da 0 a 4 per questo ce l' incremento di 1
        %per selezionare correttamente dalla Matrice B
        k=V(t+1)+1;
        Alfa(t+1,j)=S*B(j,k);
        display([ 'Alfa(' num2str(t+1) ',W' num2str(j-1) ')= S*B( W' num2str(j-1) ',V' num2str(k-1) ')  =' num2str(S) ' * ' num2str(B(j,k)) ' = ' num2str(Alfa(t+1,j)) '+'] )
        disp(s)
   end
    
end
display(['Alfa(t=' num2str(t+1) '):' ])
disp(Alfa)
disp(s) 
