...
for Mi=1:NModes
    ...
    for i=1:numR
        ...
        if npx > 5
            %Genero l'ellisse attorno ai dati SPAZIALI
            Media=mean(rc).';
            S=cov(rc);
            [V l]=eig(S);
            ...
            %Salvo le informazioni spaziali
            Region(AcceptedRegion).SMedia=Media;
            Region(AcceptedRegion).SCov=S;
            Region(AcceptedRegion).SEig=l;
            Region(AcceptedRegion).SVal =V;
            %Posso Generale la Distribuzione dei Colori nei pixel
            %selezionati
            %Genero il colore medio dei pixel specificati da r e c
            Xi=[];
            for ind=1:npx
                P=double([ Irgb(r(ind),c(ind),1) Irgb(r(ind),c(ind),1) Irgb(r(ind),c(ind),1)])./255;
                Xi=[Xi; double(P)];
            end
            Media=mean(Xi).';
            S=cov(Xi);
            ...
            Region(AcceptedRegion).CMedia=Media;
            Region(AcceptedRegion).CCov=S;
            Region(AcceptedRegion).CEig=l;
            Region(AcceptedRegion).CVal=V;
        end
    end
end
figure(6)
