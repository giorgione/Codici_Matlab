...
if ~isfield(s,'x'); %Stato Iniziale
    s.x=nan*z; 
end
if ~isfield(s,'P'); %Matrice di Covarianza Errore di Stima a Priori
    s.P=nan; 
end
if ~isfield(s,'z');  % Misura
    error('Observation vector missing'); 
end
if ~isfield(s,'u'); % Variabile di Controllo
    s.u=0;
end
if ~isfield(s,'A'); %Transizione di STato
    s.A=eye(length(x)); 
end
if ~isfield(s,'B'); % Input Control 
    s.B=0; 
end
if ~isfield(s,'Q');  % process noise covariance 
    s.Q=zeros(length(x)); 
end
if ~isfield(s,'R'); %Mat Covarianza Errore di Processo
    error('Observation covariance missing');
end
if ~isfield(s,'H'); %Mat di Misura
    s.H=eye(length(x));
end
...
if isnan(s.x)
   ...
   % Predizione di Stato
   s.x = s.A*s.x + s.B*s.u;
   % Predizione Matrice di Covarianza dell Errore a Priori
   s.P = s.A * s.P * s.A' + s.Q;
   ...
   %Stima a Posteriori de lla Covarianza dell Errore
   s.P = s.P - K*s.H*s.P;
