function [x, y, h,hArchi] = draw_layout(adj, labels, node_t, x, y)
...
for i=1:N, % Numero Nodi
  j = find(adj(i,:)==1);  %Considero solo i Nodi Adiacenti
  for k=j,
    if x(k)-x(i)==0,
	sign = 1;
    ...
    if adj(k,i)==0, % if directed edge
      ...
      myH=line([x(i)+dx1 x(k)-dx2],[y(i)+dy1 y(k)-dy2],'color','k');
      hArchi(i,k)=myH;
      hArchi(k,i)=myH;
